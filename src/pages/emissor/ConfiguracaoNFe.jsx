...
export default function ConfiguracaoNFe() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <h1 className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Configurações da NF-e</h1>
        <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>{client?.razao_social}</p>
      </div>

      <div className={`rounded-2xl shadow-sm p-6 space-y-6 ${theme === 'dark' ? 'bg-white/5 border border-white/10 backdrop-blur-md' : 'bg-white border border-gray-100'}`}>

        {/* Numeração */}
        <div>
          <h2 className={`text-base font-semibold mb-1 flex items-center gap-2 ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>
            <Settings className="w-4 h-4 text-[#0B63D4]" /> Numeração e Série
          </h2>
          <p className={`text-xs mb-4 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-400'}`}>
            Controle a série e o último número emitido para evitar duplicidades.
          </p>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={`text-sm font-semibold block mb-1 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>Série *</label>
              <input
                type="text"
                maxLength={3}
                className={`w-full rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0B63D4] ${theme === 'dark' ? 'bg-white/10 border border-white/20 text-white' : 'border border-gray-200'}`}
                placeholder="Ex: 1"
            ...
            <div>
              <label className={`text-sm font-semibold block mb-1 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>Último Número Emitido</label>
              <input
                type="number"
                min={0}
                className={`w-full rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0B63D4] ${theme === 'dark' ? 'bg-white/10 border border-white/20 text-white' : 'border border-gray-200'}`}
        ...
        <div className={`border-t pt-5 ${theme === 'dark' ? 'border-white/10' : 'border-gray-100'}`}>
          <h2 className={`text-base font-semibold mb-1 ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>Ambiente de Emissão</h2>
        ...
        <div className={`border-t pt-5 ${theme === 'dark' ? 'border-white/10' : 'border-gray-100'}`}>
          <h2 className={`text-base font-semibold mb-1 ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>NFE.io</h2>
          <div className="mt-3">
            <label className={`text-sm font-semibold block mb-1 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>Company ID no NFE.io *</label>
            <input
              type="text"
              className={`w-full rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0B63D4] ${theme === 'dark' ? 'bg-white/10 border border-white/20 text-white' : 'border border-gray-200'}`}
