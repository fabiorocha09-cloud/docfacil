  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Formas de Pagamento</h1>
            <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>{client?.razao_social} · Obrigatório pela SEFAZ</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className={`border-b ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-100'}`}>
                <th className={`text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>Nome</th>
                <th className={`text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>Tipo Pagamento (SEFAZ)</th>
                <th className={`text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>Tipo Transação</th>
                <th className={`text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>Status</th>
                <th className={`px-4 py-3 text-xs font-semibold uppercase tracking-wide ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>Ações</th>
              </tr>
            </thead>
            <tbody className={`divide-y ${theme === 'dark' ? 'divide-white/10' : 'divide-gray-50'}`}>
              <AnimatePresence>
                {itens.map(item => (
                  <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={theme === 'dark' ? 'hover:bg-white/5' : 'hover:bg-gray-50'}>
                    <td className={`px-5 py-4 font-medium ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{item.nome}</td>
                    <td className={`px-4 py-4 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>{item.tipo_pagamento}</td>
                    <td className={`px-4 py-4 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>{item.tipo_transacao}</td>
