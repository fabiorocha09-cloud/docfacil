  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Naturezas Tributárias</h1>
            <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>{client?.razao_social} · {itens.length} natureza(s)</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className={`border-b ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-100'}`}>
                <th className={`text-left px-5 py-3 text-xs font-semibold uppercase tracking-wide ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>Nome da Natureza</th>
                <th className={`text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>Finalidade</th>
              ...
              <AnimatePresence>
                {itens.map(item => (
                  <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={theme === 'dark' ? 'hover:bg-white/5' : 'hover:bg-gray-50'}>
                    ...
                    <td className={`px-4 py-4 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>{item.finalidade}</td>
                    <td className={`px-4 py-4 font-mono text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>{item.cfop_estadual} / {item.cfop_interestadual}</td>
