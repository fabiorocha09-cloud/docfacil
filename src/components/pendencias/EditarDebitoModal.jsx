...
export default function EditarDebitoModal({ debito, onClose, onSalvo }) {
  ...
  const handleSalvar = async () => {
    ...
    if (update.status !== "Pago") update.data_pagamento = null;
    await base44.entities.DebitoFiscal.update(debito.id, update);
    onSalvo();
  };
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 60, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 12, width: "100%", maxWidth: 520, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px", background: "#1B2A4A", borderRadius: "12px 12px 0 0" }}>
          <h2 style={{ margin: 0, color: "#fff", fontSize: 16, fontWeight: 700 }}>Editar Débito Fiscal</h2>
        ...
        <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div>
              ...
              <input list="tributos-edit-list" style={inp} value={form.tributo}
                onChange={e => setForm(f => ({ ...f, tributo: e.target.value }))} placeholder="Ex: IRRF, INSS..." />
              <datalist id="tributos-edit-list">
